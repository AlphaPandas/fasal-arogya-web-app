        ...
        if (nextLink !== undefined && nextLink.length > 0) {
            request({url: nextLink}, function (err, requestResponse, body) {
                ...
                for(var i = 0; i < reverseWords.length; i++) {
                    ...
                    if(keywordsPack.indexOf(reverseWords[i]) > -1) {
                        overallScore += 1
                    }
                }
                ...
                masterResponse.send(response.guess + '<br/>' + "Score : " + overallScore + "/" + response.totalScore  + '<br/>' + response.imageTagsHTML)
            })
        }
        return response
    })
